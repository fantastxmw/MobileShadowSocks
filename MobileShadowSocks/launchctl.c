 ...
 */
/*
 Modified by Tom Harrington, tph@atomicbird.com, September 27 2007.
 Modifications are based on launchctl 106.20, current as of Mac OS X 10.4.9.
...
int launchctl_unsetenv(CFStringRef key)
{
	char *fakeArgv[2];
	fakeArgv[0] = "unsetenv";
	fakeArgv[1] = (char *)CFStringGetCStringPtr(key, kCFStringEncodingUTF8);

	return (demux_cmd(2, fakeArgv) == 0);
}
...
int launchctl_setenv(CFStringRef key, CFStringRef value)
{
	...
	fakeArgv[0] = "setenv";
	fakeArgv[1] = (char *)CFStringGetCStringPtr(key, kCFStringEncodingUTF8);
	fakeArgv[2] = (char *)CFStringGetCStringPtr(value, kCFStringEncodingUTF8);
	
	return (demux_cmd(3, fakeArgv) == 0);
}
...
static void _launchctl_append_env(launch_data_t obj, const char *key, void *context)
{
	CFMutableDictionaryRef launchdEnivronment = (CFMutableDictionaryRef)context;
	CFStringRef keyCF = CFStringCreateWithCString(kCFAllocatorDefault, key, kCFStringEncodingUTF8);
	CFStringRef valueCF = CFStringCreateWithCString(kCFAllocatorDefault, launch_data_get_string(obj), kCFStringEncodingUTF8);
	CFDictionaryAddValue(launchdEnivronment, keyCF, valueCF);
	CFRelease(keyCF);
	CFRelease(valueCF);
}
...
CFDictionaryRef launchctl_export()
{
	...
	CFMutableDictionaryRef launchdEnivronment = CFDictionaryCreateMutable(kCFAllocatorDefault, 0, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);
	...
	if (resp) {
		launch_data_dict_iterate(resp, _launchctl_append_env, launchdEnivronment);
		launch_data_free(resp);
	} else {
		fprintf(stderr, "launch_msg(\"" LAUNCH_KEY_GETUSERENVIRONMENT "\"): %s\n", strerror(errno));
	}
	return launchdEnivronment;
}

CFStringRef launchctl_getenv(CFStringRef key)
{
	CFDictionaryRef launchdEnvironment = launchctl_export();
	CFStringRef value;
	
	value = CFDictionaryGetValue(launchdEnvironment, key);
	CFRetain(value);
	CFRelease(launchdEnvironment);
	return value;
}
...
static int _launchctl_load_unload_path(bool loadFlag, CFStringRef plistPath, bool writeFlag, bool forceFlag)
{
	// I do things this way so as to minimize the changes to the original launchctl code.
	// load_and_unload_cmd wants to read argc and argv, so that's what I give it.
	CFMutableArrayRef fakeArgvCF = CFArrayCreateMutable(kCFAllocatorDefault, 0, NULL);
	if (loadFlag)
		CFArrayAppendValue(fakeArgvCF, "load");
	else
		CFArrayAppendValue(fakeArgvCF, "unload");
	if (writeFlag)
		CFArrayAppendValue(fakeArgvCF, "-w");
	if (forceFlag)
		CFArrayAppendValue(fakeArgvCF, "-F");
	char path[MAXPATHLEN];
	CFStringGetFileSystemRepresentation(plistPath, path, MAXPATHLEN);
	CFArrayAppendValue(fakeArgvCF, path);
	
	int fakeArgc = CFArrayGetCount(fakeArgvCF);
	const void **fakeArgv = malloc(sizeof(void*) * fakeArgc);
	CFArrayGetValues(fakeArgvCF, CFRangeMake(0, CFArrayGetCount(fakeArgvCF)), fakeArgv);
	int result = demux_cmd(fakeArgc, (char **)fakeArgv);
	free(fakeArgv);
	return result;
}
...
{
	return _launchctl_load_unload_path(true, plistPath, writeFlag, forceFlag);
}

int launchctl_unload_path(CFStringRef plistPath, bool writeFlag)
{
	return _launchctl_load_unload_path(false, plistPath, writeFlag, false);
}
...
static int launchctl_start_stop(bool startFlag, CFStringRef jobLabel)
{
	CFMutableArrayRef fakeArgvCF = CFArrayCreateMutable(kCFAllocatorDefault, 5, NULL);
	if (startFlag)
		CFArrayAppendValue(fakeArgvCF, "start");
	else
		CFArrayAppendValue(fakeArgvCF, "stop");
	CFArrayAppendValue(fakeArgvCF, CFStringGetCStringPtr(jobLabel, kCFStringEncodingUTF8));
	
	int fakeArgc = CFArrayGetCount(fakeArgvCF);
	const void **fakeArgv = malloc(sizeof(void*) * fakeArgc);
	CFArrayGetValues(fakeArgvCF, CFRangeMake(0, CFArrayGetCount(fakeArgvCF)), fakeArgv);
	int result = (demux_cmd(fakeArgc, (char **)fakeArgv) == 0);
	free(fakeArgv);
	return result;
...
{
	return launchctl_start_stop(true, jobLabel);
}

int launchctl_stop(CFStringRef jobLabel)
{
	return launchctl_start_stop(false, jobLabel);
}
...
static void _launchctl_append_jobs(launch_data_t j __attribute__((unused)), const char *label, void *context)
{
	CFMutableArrayRef launchdList = (CFMutableArrayRef)context;
	CFStringRef labelCF = CFStringCreateWithCString(kCFAllocatorDefault, label, kCFStringEncodingUTF8);
	CFArrayAppendValue(launchdList, labelCF);
	CFRelease(labelCF);
}

CFArrayRef launchctl_list()
{
	// Unlike some of the other launchctl API functions, this one doesn't just call the existing launchctl code.
	// The launch_data_dict_iterate semantics made it impossible.
	launch_data_t resp, msg;
	CFMutableArrayRef launchdList = CFArrayCreateMutable(kCFAllocatorDefault, 0, &kCFTypeArrayCallBacks);
	...
	if (resp == NULL) {
		fprintf(stderr, "launch_msg(): %s\n", strerror(errno));
		CFRelease(launchdList);
		return NULL;
	} else if (launch_data_get_type(resp) == LAUNCH_DATA_DICTIONARY) {
		launch_data_dict_iterate(resp, _launchctl_append_jobs, launchdList);
	} else {
		fprintf(stderr, "%s launchd returned unknown response\n", getprogname());
		CFRelease(launchdList);
		launchdList = NULL;
	}
